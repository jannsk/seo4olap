...
public class BaseCubeSparqlDerivedDatasetIterator implements
	...
	public BaseCubeSparqlDerivedDatasetIterator(OlapSesameEngine engine,
			String dataseturi) {
...
//			}
	...
	public void init() throws Exception {
		...
		if(engine.getBaseCube() == null){
			Olap4ldUtil._log.info("derive new Basecube");
			baseCube = sparql(this.query);
			engine.setBaseCube(baseCube);
		}
		else{
			Olap4ldUtil._log.info("derive Basecube from LinkedDataEngine");
			baseCube = engine.getBaseCube();
		}
		this.outputiterator = baseCube.iterator();
	}
